...
def conv(model,
  ...
  wav, _ = librosa.load(os.path.join(voice_path_s, voice_file), sr = sampling_rate, mono = True)
  ...
  sf.write(os.path.join(output_dir, voice_dir_list[s_label]+"_to_"+voice_dir_list[t_label]+"_["+voice_file+"].wav"),
            wav_transformed, 
            sampling_rate)
  wav_source = world_speech_synthesis(f0 = f0_converted, sp = sp, ap = ap, fs = sampling_rate, frame_period = frame_period)
  sf.write(os.path.join(output_dir, voice_dir_list[s_label]+"_to_"+voice_dir_list[t_label]+"_["+voice_file+"]_nonconv.wav"),
            wav_source, 
            sampling_rate)
